    ...
    while True:
        try:
            ...
            if player.name == "white":
                ...
                board, piece, move_opt = copy_piece_move(
                    board, p_piece, p_move_opt)
            else:
            ...
            if (not piece) or (not move_opt):
                ...
                if res:
                    ...
                    print(
                        f"=== Game ended: {player.name} can not make a legal move ===")
                break
                ...
                try:
                    piece.move(move_opt)
                    print(
                        f"{piece} move to: ({move_opt.position.x},{move_opt.position.y})")
                    if getattr(move_opt, "captures", None):
                        caps = ", ".join(
                            f"({c.x},{c.y})" for c in move_opt.captures)
                        if caps:
